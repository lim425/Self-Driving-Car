...
from sdc_venv_pkg.Detection.Signs.c_Tracking.optical_flow_tracking import Tracker
...
def sign_detection_and_track(gray_img, frame, frame_draw):
    ...
    if (sign_tracker.mode == "Detection"):
                ...
                try:
                    ...
                    if model is not None:
                        ...
                        if(sign != "No_Sign"):
                            ...
                            # >>>>>>>>>>>>>>>>>>> 4a. If match found, Increment ... known centers confidence >>>>>>>>>>>>>>>>>>>
                            if match_found:
                                ...
                                # >>>>>>>>>>>>>>>>>>> 4b. Check if same sign detected 3 times, if yes initialize Optical Flow Tracker >>>>>>>>>>>>>>>>>>>
                                if (sign_tracker.known_centers_confidence[match_idx] > 4):
                                    ...
                                    print("The sign we are about to track is most likely ( " + str(max_value), " ) a " + sign_classes[max_index])
                                    sign_tracker.init_tracker(sign_classes[max_index], gray_img, frame_draw, startP, endP)
                            # >>>>>>>>>>>>>>>>>>> 4c. If sign detected first time ... Update sign detection and its detected count >>>>>>>>>>>>>>>>>>>
                            else:
        ...
        # Tracking Mode
        sign_tracker.track(gray_img, frame_draw)
        # display class
