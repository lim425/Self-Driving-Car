...
import math
import time
# from self_driving_car_pkg.Detection.Lanes.utilities import Distance, Distance_
# from self_driving_car_pkg.config import config
# from .utilities import Distance, Distance_
# from ...config import config

# venv path lib
from sdc_venv_pkg.Detection.Lanes.utilities import Distance, Distance_
from sdc_venv_pkg.config import config

def BwareaOpen(img,MinArea):   #remove the small noise

    thresh = cv2.threshold(img, 0, 255, cv2.THRESH_BINARY)[1]
    # Filter using contour area and remove small noise
    cnts = cv2.findContours(thresh, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)[0]  #original [1]
    cnts_TooSmall = []
...
def RetLargestContour_OuterLane(gray,minArea):   # return countour of outerlane (yellow)
    LargestContour_Found = False
    thresh=np.zeros(gray.shape,dtype=gray.dtype)
    _,bin_img = cv2.threshold(gray,0,255,cv2.THRESH_BINARY)
    ...
    cnts = cv2.findContours(bin_img, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)[0]   #original [1]
    Max_Cntr_area = 0
    Max_Cntr_idx= -1
    for index, cnt in enumerate(cnts):
        area = cv2.contourArea(cnt)
...
def Ret_LowestEdgePoints(gray):    # return inner line(contour) of the outerlane(yellow)
    ...
    _,bin_img = cv2.threshold(gray,0,255,cv2.THRESH_BINARY)
        #Find the two Contours for which you want to find the min distance between them.
    cnts = cv2.findContours(bin_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)[0]  #original[1]
    ...
    cnts2 = cv2.findContours(Contour_TopBot_PortionCut, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)[0]  #original[1]
